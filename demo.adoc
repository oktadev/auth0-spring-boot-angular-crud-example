...
. Add a route for the `HomeComponent` and `GroupListComponent` to `app.routes.ts`: [`sba-routes`]
+
[source,typescript]
----
...
. Update `app/cypress/support/commands.ts` to add a `login(username, password)` method: [`sba-commands`]
+
.`commands.ts`
[%collapsible]
====
...
. Update `app/cypress/support/e2e.ts` to log in before each test and log out after. [`sba-e2e`]
+
.`e2e.ts`
[%collapsible]
====
...
. Add a `app/cypress/e2e/home.cy.ts` test to verify that the home page loads. [`sba-e2e-home`]
+
[source,ts]
----
describe('Home', () => {
...
. Create a `groups.cy.ts` in the same directory to test CRUD on groups. [`sb-e2e-groups`]
+
.`groups.cy.ts`
[%collapsible]
====
...
. If you run `npm test`, you'll see several failures. Fix them:
  - Update `home.component.spec.ts` to import `HttpClientTestingModule`
  - Update `app.component.spec.ts` to import `MatToolBarModule` and look for `JUG Tours`
  - Update group components to import `HttpClientTestingModule` and `RouterTestingModule`

. Run `mvn test` without setting environment variables and your tests will fail.
  - Add `TestSecurityConfiguration` to mock an OAuth provider [`sba-test-config`]
  - Update `JugToursApplicationTests`:
+
[source,java]
----
...
. Add a GitHub workflow at `.github/workflows/main.yml` to prove that your tests run in CI. [`sba-ci`]
+
.`main.yml`
[%collapsible]
====
...
. Push your changes to GitHub and watch the https://github.com/oktadev/auth0-spring-boot-angular-crud-example/actions/workflows/main.yml[main workflow] run. ✅
